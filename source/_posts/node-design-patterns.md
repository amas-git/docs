...
经过Promise我们终于可以把逐层嵌套的callback 改写成单层的then方法序列。但是这样写仍然很啰嗦，每处理一个任务，就需要写一个then方法和一个callback。我们想要继续简化代码就需要ES6提供的async/await关键字了。在两个语法糖的帮助下，我们可以这么写：
...
- await只能在async函数中被使用
- 用async修饰的函数总是返回一个Promise对象，即便你return的不是Promise对象，也会被转换为Promise对象。
- 使用await你就可以等待一个Promise执行完毕再继续执行下面的代码

### Generator方式

generator是一种特殊的函数，普通的函数要么不执行，要么全部执行。generator则可以部分的执行,  通过一个叫做yield的关键字你可以暂停函数的执行并且返回一结果, 通过geerator的next方法可以让原来暂停的函数继续运行。
...
假如有一杯水，一口锅，将这杯水烧开是很容易的。我们只需要将水倒入锅中加热即可。但如果换做是一浴缸的水，一个湖的水我们采用同样的方法就会很难了。简单的造一口更大的锅不仅困难，而且很不划算。因为金属是远远比水要难于获取。这时候我们可以一锅一锅的烧，直到把所有的水烧开。我们甚至可以建立一个金属管道把水流不断引入这个管道，然后加热这个管道使水流流出管道时正好加热到100度。这个就是流的基本思想，当我们处理的数据规模不断增加的时，我们并不需要同等规模的扩大内存，我们只需要使用有限的内存分多次把所有的数据处理完毕就好。流的另外一个好处是，我们不必等到所有的数据全部处理完毕才拿到结果，而是处理一部分得到一部分使用一部分。现实生活中到处都是流。
...
> R.pipe(T1).pipe(T2) ... .pipe(Tn).pipe(W)
>
> R: Readable Stream T: Transform Stream W:Writeable Stream	

### 流的缓冲区

流的核心结构就是缓冲区。缓冲区本质就是一个数组。如果流的最小处理单位是字符，那么就可以使用string作为缓冲区。如果最小处理单位是字节，则可以使用Buffer作为缓冲区。如果最小处理单位是对象，则可以使用普通的数组作为缓冲区。

- 缓冲区的大小由highWaterMark获得，在构建流的时候可以设置它的大小
- 
